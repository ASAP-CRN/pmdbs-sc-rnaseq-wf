...
def filter_adata(adata: AnnData) -> AnnData:
    ...
    mu.pp.filter_obs(adata, "total_counts", lambda x: (x >= total_counts_limits[0]) & (x <= total_counts_limits[1]))
    mu.pp.filter_obs(adata, "n_genes_by_counts", lambda x: (x >= n_genes_by_counts_limits[0]) & (x <= n_genes_by_counts_limits[1]))
